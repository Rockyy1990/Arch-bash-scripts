...
# WICHTIG:
#   - 'bios.syslinux'                    → aktueller BIOS-Bootmode (nicht mehr .mbr/.eltorito)
#   - 'uefi-x64.systemd-boot.esp'        → aktueller UEFI-Bootmode (systemd-boot, kein GRUB!)
#   - 'uefi-x64.systemd-boot.eltorito'   → für El-Torito (optisches Medium / dd)
#   - KEIN uefi-ia32.grub.* / uefi-x64.grub.* → diese sind deprecated und erfordern grub auf dem Host
...
# bios.syslinux          → BIOS/Legacy Boot via Syslinux
# uefi-x64.systemd-boot.esp       → UEFI x64 via systemd-boot (ESP)
# uefi-x64.systemd-boot.eltorito  → UEFI x64 via systemd-boot (El Torito / optisch)
bootmodes=(
    'bios.syslinux'
...
ok "profiledef.sh geschrieben (systemd-boot, kein GRUB)."
...
# multilib aktivieren (für lib32-Pakete wie lib32-vulkan-*)
...
mkdir -p "${AIROOTFS}/etc/locale.conf.d"
...
#            Moderner Weg: direkt in airootfs – KEIN customize_airootfs.sh
# =============================================================================
banner "Schritt 6: Benutzer-Accounts konfigurieren"

# ── /etc/passwd ───────────────────────────────────────────────────────────────
# Bestehende passwd aus releng erweitern
# liveuser hinzufügen (UID 1000)
...
# ── /etc/group ────────────────────────────────────────────────────────────────
# Gruppen anlegen / liveuser eintragen
...
for grp in "${!GROUPS_MAP[@]}"; do
    ...
    if ! grep -q "^${grp}:" "${AIROOTFS}/etc/group" 2>/dev/null; then
        ...
        # liveuser zur Gruppe hinzufügen falls noch nicht drin
        if ! grep -q "^${grp}:.*${LIVE_USER}" "${AIROOTFS}/etc/group" 2>/dev/null; then
            sed -i "/^${grp}:/ s/$/,${LIVE_USER}/" "${AIROOTFS}/etc/group"
            # Doppelkomma am Anfang der Mitgliederliste vermeiden
...
# =============================================================================
# SCHRITT 10: Systemd-Services aktivieren (via systemd preset / symlinks)
# =============================================================================
...
# =============================================================================
# SCHRITT 13: NetworkManager Konfiguration
# =============================================================================
banner "Schritt 13: NetworkManager konfigurieren"
...
# =============================================================================
# SCHRITT 14: Polkit-Regel für liveuser
# =============================================================================
banner "Schritt 14: Polkit-Regel"
...
# =============================================================================
# SCHRITT 15: systemd-boot Konfiguration (efiboot/)
#             Wird aus dem releng-Profil übernommen – nur Anpassungen
# =============================================================================
banner "Schritt 15: systemd-boot Konfiguration prüfen"

EFIBOOT="${PROFILE_DIR}/efiboot"
if [[ -d "${EFIBOOT}" ]]; then
    ok "efiboot/-Verzeichnis aus releng vorhanden (systemd-boot)."
    # loader.conf anpassen: Timeout auf 5 Sekunden
...
# =============================================================================
# SCHRITT 16: syslinux Konfiguration prüfen (BIOS)
# =============================================================================
banner "Schritt 16: syslinux Konfiguration prüfen"
...
# =============================================================================
# SCHRITT 17: Arbeits- und Ausgabeverzeichnisse vorbereiten
# =============================================================================
banner "Schritt 17: Verzeichnisse vorbereiten"
...
# =============================================================================
# SCHRITT 18: Profil-Validierung (Kurzcheck vor dem Build)
# =============================================================================
banner "Schritt 18: Profil-Validierung"
...
# =============================================================================
# SCHRITT 19: ISO BAUEN
# =============================================================================
banner "Schritt 19: ISO-Build starten"
...
if [[ -n "${ISO_FILE}" && -f "${ISO_FILE}" ]]; then
    ...
    echo "  # oder einfacher:"
    echo "  cp ${ISO_FILE} /dev/sdX"
    echo ""
    echo -e "${CYAN}  ── SHA256-Prüfsumme speichern: ──────────────────────────${NC}"
    echo "  echo '${SHA256}  ${ISO_FILE}' > ${ISO_FILE}.sha256"
    ...
    if [[ -n "${SUDO_USER}" && -n "${ISO_FILE}" ]]; then

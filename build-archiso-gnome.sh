#!/usr/bin/env bash
# =============================================================================
#  build-archiso-gnome.sh
#  Erstellt eine Arch Linux ISO mit GNOME Desktop (Wayland) + Autologin
#
#  Basiert auf dem aktuellen archiso releng-Profil mit systemd-boot (UEFI)
#  und syslinux (BIOS) – keine deprecated GRUB-Bootmodes.
#
#  Ausführung: sudo bash build-archiso-gnome.sh
...
# ─── Konfiguration ────────────────────────────────────────────────────────────
readonly PROFILE_DIR="/tmp/archgnome"
...
iso_label="ARCH_GNOME_$(date --date="@${SOURCE_DATE_EPOCH:-$(date +%s)}" +%Y%m)"
iso_publisher="Custom Arch Linux GNOME Build"
iso_application="Arch Linux GNOME Live"
...
# =============================================================================
# SCHRITT 8: GDM Autologin (GNOME Wayland)
# =============================================================================
banner "Schritt 8: GDM Autologin konfigurieren"
...
ok "GDM Autologin für GNOME Wayland konfiguriert."

# =============================================================================
# SCHRITT 9: dconf-Defaults für GNOME (Tastatur, Sprache etc.)
# =============================================================================
banner "Schritt 9: GNOME dconf-Defaults"

cat > "${AIROOTFS}/etc/dconf/profile/user" << 'EOF'
user-db:user
system-db:local
EOF

cat > "${AIROOTFS}/etc/dconf/db/local.d/00-keyboard" << 'EOF'
...
[org/gnome/shell]
favorite-apps=['firefox.desktop', 'org.gnome.Nautilus.desktop', 'org.gnome.Terminal.desktop', 'org.gnome.TextEditor.desktop']
EOF

ok "dconf-Defaults geschrieben."
...
ok "Pacman-Hook für locale-gen geschrieben."
...
echo -e "${BOLD}${GREEN}  Fertig! Viel Spaß mit deiner Arch Linux GNOME ISO!     ${NC}"
echo -e "${BOLD}${GREEN}══════════════════════════════════════════════════════════${NC}"
echo ""
